...
create table if not exists public.pitch_analysis_results (
  ...
  student_id uuid not null references public.students(id) on delete cascade,
  assignment_id uuid not null references public.assignments(id) on delete cascade,
  voice_type text not null default 'female' check (voice_type in ('female', 'male')),
  written_target_pitch_json jsonb not null default '[]'::jsonb,
  expected_sounding_pitch_json jsonb not null default '[]'::jsonb,
  raw_detected_pitch_json jsonb not null default '[]'::jsonb,
  scoring_adjusted_pitch_json jsonb not null default '[]'::jsonb,
  target_pitch_json jsonb not null default '[]'::jsonb,
